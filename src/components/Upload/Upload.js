...
export default class Upload extends Component {
  ...
  handleFinish = e => {
    ...
    if (!this.context.id) {
      ...
      ExperimentApiService.postExperimentRegions(
        this.context.regions.experiment_id,
  ...
  handleImageSend = image => {
    ExperimentApiService.postExperimentImage(
      this.context.id,
      image,
      ...
      stage.getContent().addEventListener("wheel", wheelEvent => {
        if (this.state.uploaded) {
          ...
          const deltaYBounded = !(wheelEvent.deltaY % 1)
            ? Math.abs(Math.min(-100, Math.max(100, wheelEvent.deltaY)))
            : Math.abs(wheelEvent.deltaY);
          const scaleBy = 0.99 + deltaYBounded / 1000;
          const newScale =
      ...
      stage.getContent().addEventListener(
        ...
        evt => {
          ...
          if (t1 && t2) {
            ...
            const pointer =
              point || this.clientPointerRelativeToStage(px, py, stage);
  ...
  updateScaleBoxOffset = () => {
    this.setState({
      scaleBoxOffset: this.containerRef.current.offsetHeight / 2
    });
  };
  ...
  outsideImageBounds = point => {
    if (
      point.x < 0 ||
      point.y < 0 ||
      point.x > this.context.image.width ||
      point.y > this.context.image.height
    ) {
      return true;
    } else {
      return false;
  ...
  createCellRegion = e => {
    if (this.state.uploaded) {
      ...
      if (this.outsideImageBounds(region.point)) {
        ...
        this.context.setRegions(
          this.context.regions.regions.data.concat(region)
        );
      ...
      <>
        ...
        <div className="upload">
          <div id="container" ref={this.containerRef}>
            ...
            {!this.state.uploaded && (
              <form id="imageform" className="uploadform">
            ...
            >
              ...
              <Layer ref={this.regionsLayerRef}></Layer>
            </Stage>
          </div>
          ...
          {this.context.regions.regions.data.length >= 0 &&
            this.state.uploaded && (
              <>
                <RegionsList
                  onClick={this.findCellOnStage}
                  regions={this.context.regions.regions.data}
                />
                <button
